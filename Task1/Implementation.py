...
class Adaline:
    def _init_(self,eta=0.01,epochs=100,threshold=0.01) :
        self.eta=eta
        self.epochs=epochs
        self.weights=None
        self.bias=False
    ...
    def training(self,X,y):
        ...
        for _ in range(self.epochs):
            ...
            for x,target in zip(X,y):
                if self.bias:
                  y_hat=np.dot(X,self.weights.T)+self.bias
                else:
                  y_hat=np.dot(X,self.weights.T)    
                error=target-y_hat
                self.weights+=self.eta*error*x
                self.bias+=self.eta*error
                mse+=error**2
            self.mse_err.append(mse/2*len(y))
            if mse/2*len(y)<self.threshold:
                break
